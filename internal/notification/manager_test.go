...
func TestManager_AddNotifier_Multiple(t *testing.T) {
	...
	mgr.AddNotifier("slack", &mockNotifier{name: "slack"})
...
func TestManager_AddNotifier_Replace(t *testing.T) {
	...
	notifier1 := &mockNotifier{name: "test"}
	notifier2 := &mockNotifier{name: "test"}
	...
	assert.Equal(t, "test", notifiers[0].Name, "expected replacement notifier to be used")
}
...
func TestManager_NotifierCount(t *testing.T) {
	...
	mgr.AddNotifier("a", &mockNotifier{name: "a"})
	assert.Equal(t, 1, mgr.NotifierCount())

	mgr.AddNotifier("b", &mockNotifier{name: "b"})
...
func TestManager_ListNotifiers(t *testing.T) {
	...
	assert.True(t, found["telegram"], "expected telegram notifier")
	assert.True(t, found["discord"], "expected discord notifier")
}
...
func TestManager_ConcurrentAddAndNotify(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		go func(id int) {
			...
			mgr.AddNotifier(name, &mockNotifier{name: name})
			done <- true
		}(i)
	}

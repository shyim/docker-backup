...
docker-backup daemon \
  ...
  --notify=<provider>.<option>=<value> # Define notification providers
...
Notification providers can be configured via CLI flags or environment variables. CLI flags take precedence.
...
--notify=discord.username=Docker Backup  # Optional, defaults to "Docker Backup"
```
...
Provider names are uppercase, options use underscores (converted to hyphens internally).
...
func (t *SlackType) Name() string { return "slack" }

func (t *SlackType) Create(name string, options map[string]string) (notification.Notifier, error) {
    webhookURL := options["webhook-url"]
    if webhookURL == "" {
        return nil, fmt.Errorf("slack notifier %q requires 'webhook-url' option", name)
    }
    return &SlackNotifier{name: name, webhookURL: webhookURL}, nil
}
...
Then import in `internal/notifiers/registry.go`.
